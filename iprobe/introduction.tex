...
% compared to user level solutions (we compare overheads of iProbe in later sections).
...
In this paper, we introduce a dynamic instrumentation framework called \emph{iProbe}.
...
%profiling resource usage at a function level granularity by sampling hardware performance counters using iProbe.
...
%Section \ref{sec:intro} introduces the motivation and key contributions behind iProbe. 
...
%Its implementation details are presented in Section \ref{sec:implementation}. 
%The evaluation of iProbe is demonstrated in Section \ref{sec:eval}. 
