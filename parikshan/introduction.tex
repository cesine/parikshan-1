...
While these systems impose almost no overhead on the production system being debugged (since they simply collect log information already being collected, or have light-weight monitoring), they are limited in their fault finding and reproduction power, hence limited in their utility to developers.
...
We seek to allow developers to diagnose and resolve crashing and non-crashing failures of production service-oriented systems \emph{without suffering any performance overhead due to instrumentation}.
...
\item \textbf{No Instrumentation Overhead on Production System:} While existing approaches have focused on minimizing the recording overhead. \parikshan uses novel non-blocking network duplication to avoid any overhead at all in the production environment due to the instrumentation itself.	\footnote{There may be a negligible run-time overhead because asynchronous memory copy operation in network forwarding.}
